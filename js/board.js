...
function renderSingleTaskCard(statusId, taskId, container) {
	...
	container.innerHTML += renderTaskCard(
		statusId,
		taskId,
		cat,
		catColor,
...
function getSubtasksDone(statusId, taskId) {
	return tasks[statusId][taskId]["subtasks"].filter((item) => item["status"])
		.length;
}
...
async function openAddTask() {
	await loadHTML(
		"modal-add-task-content",
		"./assets/templates/add_task__template.html"
	);
...
function searchTasks() {
	const searchTerm = document
		.getElementById("find-task--input")
		.value.toLowerCase();
	searchResult = [[], [], [], []];
	...
	for (let i = 0; i < tasks.length; i++) {
		for (let t = 0; t < tasks[i].length; t++) {
			if (
				tasks[i][t]["title"].toLowerCase().indexOf(searchTerm) >= 0 ||
	...
	if (searchResult[statusId].length == 0) {
		...
		for (let t = 0; t < searchResult[statusId].length; t++) {
			renderSingleTaskCard(
				searchResult[statusId][t].statusId,
